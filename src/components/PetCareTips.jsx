  ...
  return (
    <section className="mx-auto w-11/12 my-30 md:w-9/12">
      ...
      <div className="mt-10 grid gap-6 md:grid-cols-3">
        {tips.map((tip) => (
          <article
            className="rounded-lg border border-[#fb756326] bg-white p-6 text-center shadow-sm transition hover:-translate-y-1 hover:shadow-md"
            key={tip.title}
          >
            <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-lg bg-[#fb75631a]">
              {tip.icon}
            </div>
            <h3 className="mt-5 text-lg font-semibold text-[#2e2804]">{tip.title}</h3>
